...
enemy_bullets = []
enemy_bullet_width, enemy_bullet_height = 15, 15  # Start smaller
enemy_bullet_speed = 3  # Slower enemy bullets
enemy_bullet_size_multiplier = 1
enemy_bullet_increase = 5  # Increase by 5 pixels each time
...
def main():
    ...
    global enemy_jumping, enemy_y, enemy_vel_y
    global level  # <-- Add this line
    ...
    while running:
        ...
        for e_bullet in enemy_bullets[:]:
            if player_rect.colliderect(e_bullet):
                ...
                hit_sound.play()  # Play hit sound here
                score -= 5  # Lose 5 points per hit
                if score < 0:
                    score = 0  # Prevent negative score
                enemy_bullets.remove(e_bullet)  # Remove bullet after hit
                break
        ...
        for bullet in bullets[:]:
            if enemy_rect.colliderect(bullet):
                ...
                if score % 10 == 0:
                    enemy_bullet_size_multiplier += 1
                    enemy_bullet_width = 10 + enemy_bullet_increase * (enemy_bullet_size_multiplier - 1)
                    enemy_bullet_height = 10 + enemy_bullet_increase * (enemy_bullet_size_multiplier - 1)
